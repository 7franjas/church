...
@section('title','Modificar Diezmo')
@section('content_header','Modificar Diezmo Nº'.$diezmo->id)
@section('content_description','Modificar la información del diezmo')
    ...
    <div class="col-md-9">
      <div class="box box-primary">
        ...
        <div class="box-header with-border box-header-color">
          <h3 class="box-title">Modificar Información | Diezmo</h3>
        </div>

        <div class="box-body">
          <div class="form-container">

            <form action="{{ route('diezmos.update', $diezmo->id) }}" method="POST">
              ...
              <div class="field {{ $errors->has('brother_name') ? 'has-error' : '' }}">
                <div class="input-group">
                  <input id="brother_name" name="brother_name" type="text" class="form-control form-data" placeholder="Nombre" value="{{ $diezmo->brother->name }}">
              ...
              <div class="field {{ $errors->has('monto') ? 'has-error' : '' }}">
                <div class="input-group">
                  <input id="monto" name="monto" type="monto" class="form-control form-data" placeholder="Monto" value="{{ $diezmo->monto }}">
              ...
              <div class="field {{ $errors->has('date') ? 'has-error' : '' }}">
                <div class="input-group">
                  <input id="datepicker"  id="date" name="date" type="fecha" class="form-control form-data" placeholder="Fecha" value="{{ $diezmo->date }}">
                  <span class="input-group-addon"><i class="glyphicon glyphicon-envelope"></i></span>
                </div>
              ...
              <div class="field {{ $errors->has('observation') ? 'has-error' : '' }}">
                <div class="input-group">
                  <textarea id="observation" name="observation" type="text" class="form-control form-data" placeholder="Observaciones" value="{{ $diezmo->observation }}">{{ $diezmo->observation }}</textarea>
                  <span class="input-group-addon"><i class="glyphicon glyphicon-list-alt"></i></span>
                </div>
                @if($errors->has('observation'))<span class="help-block">{{ $errors->first('observation') }}</span>@endif
              </div>

              <input id="brother_id" name="brother_id" type="hidden" value="{{ $diezmo->brother_id }}">
              

              <div class="form-button">
                <button type="submit" class="btn btn-primary">Modificar Diezmo</button>
              </div>
