...
class DiezmoDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('action', function($query){
            return  '<a href="'.route("diezmos.show", $query->id).'" class="label label-success action-show" title="Ver"><i class="fa fa-eye"></i></a>'.
                    '<a href="'.route("diezmos.edit", $query->id).'" class="label label-warning action-edit" title="Editar"><i class="fa fa-edit"></i></a>'.
    ...
    public function query(Ingreso $model)
    {
        ...
        $ingreso = $model->where('typeIng',1)->select();
