...
class FamilyDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('action', function($query){
            return  '<a href="'.route("familys.show", $query->id).'" class="label label-success action-show" title="Ver"><i class="fa fa-eye"></i></a>'.
                    '<a href="'.route("familys.edit", $query->id).'" class="label label-warning action-edit" title="Editar"><i class="fa fa-edit"></i></a>'.
                    '<a href="javascript:void(0)" onclick="modalDestroy(event,'.$query->id.',\''.$query->fam_name.'\')" class="label label-danger action-destroy" title="Eliminar"><i class="fa fa-remove"></i></a>';
        })
    ...
    public function query(Family $model)
    {
        ...
        $family = $model->newQuery()->select();

        return $this->applyScopes($family);
    ...
    protected function getColumns()
    {
        return [
            ...
            'fam_name' => ['name'=>'fam_name', 'data'=>'fam_name', 'title'=>'Familia', 'visible'=>true],
            'fam_observation' => ['name'=>'fam_observation', 'data'=>'fam_observation', 'title'=>'Observaciones', 'visible'=>true],
            ...
            'deleted_at' => ['name'=>'deleted_at', 'data'=>'deleted_at', 'title'=>'Eliminado en', 'visible'=>false]
        ];
    }
